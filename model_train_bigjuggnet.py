...
import os
import logging
from make_icdar2013_list import make_testset, make_trainset
...
class configure(object):
    def __init__(self, f='train', name='Big_Jugg_net_Hebei_pk'):
        self.makeset = False
        self.net_size = 0.5 ##?应该和后面的cnn_size 一致，表示 channel 数量的折减倍率。
        self.withequdata = False ##样本均衡。保证一个batch 的样本中，各个类别的样本数量一致。
        self.lr_scheduler = mx.lr_scheduler.FactorScheduler(step=450, factor=0.99) ##学习计划，每隔450，变为原来的0.99
        self.epochload = 300
#        self.trainpath = 'yellowplate_train_string.lst' ##训练集。withequdata = False ##.lst文件样例：第一列是index，第二列是label，第三列是图像路径
#        self.trainpath = 'data-Clean1/trainData_Big_Jugg_net_Clean1.lst' ##训练集。withequdata = False ##.lst文件样例：第一列是index，第二列是label，第三列是图像路径
        self.trainpath = '/data/huzechen/work/plate_OCR/Hebei_pk_train.lst' ##训练集。withequdata = False ##.lst文件样例：第一列是index，第二列是label，第三列是图像路径
#        self.path = '/data/huzechen/rectify2.0.0.1_recog_data/data_list' ##训练集。withequdata = True
#        self.testpath = "yellowplate_train_string.lst" ##从训练集中选取部分作为测试集。
        self.testpath = '/data/huzechen/work/plate_OCR/Hebei_pk_test.lst' ##从训练集中选取部分作为测试集。
        
        
        self.function = f ## ‘train’ 或者 ‘test’ 操纵。
        self.configname = name ##网络结构的名字。sym_gen中根据名字来做相应的操作
        self.lr = 0.0001 ##初始学习速率
        self.clip_gradient = 0.01 ##梯度模长限制。错误样本的梯度较大，通过比例回调来限制其梯度。类似地可以限制梯度在一定的区间内
        self.optimizer = 'sgd' ##优化器 sgd 或者 adam
        self.num_epoch = 500 ##训练迭代的次数
...
#                                   'deepv_train': 3, 'doubrows_train': 2,
#                                   '13province_train+kakou_train+xunzhimei_train+qujing_train+shoufeizhan_train': 1}
#        ##样本平衡。字典，记录每个样本所占的比例。总和为8，2代表2/8。按照比例挑选 batch。
...
#        if self.function == 'timetest': ##timetest计算每次运算需要的时间
#            self.batchsize = 1
#            self.contexts = [mx.gpu(0)]
            
#        if 'lstm' in self.configname:
#            self.num_hidden = 256 ##定义隐藏层的神经元数量
#            self.num_lstm_layer = 2 ##定义lstm层数

#from Jugg_net import Jugg_net #输入各种类型的 lstm 神经元
from text_lstm_resnet50 import  Big_Jugg_net#输入各种类型的 lstm 神经元
#from text_bucketing_iter_debug import TextIter
from convnet import  convnet, no_slice_convnet
#from text_bucketing_iter_yyh_v2 import TextIter
from text_bucketing_iter_blstm import TextIter
from EquDataIter import newTextIter

import time
import cv2, random
from io import BytesIO ##中文字符计数。一个中文字符3位

config = configure() ##默认是 f='train', name='Jugg_net_1_halfconv'
...
logging.basicConfig(level=logging.DEBUG, ##设置日志级别，默认为logging.WARNING
                ...
                filename=config.configname+'.log', ##日志文件格式
                filemode='a+') ##模式是写入
...
if __name__ == '__main__':
    ...
    if not config.withequdata and config.makeset: ##withequdata = False，makeset = True
        make_trainset() 
        make_testset()
...
#    def conv_sym_gen(seq_len):
#        return conv(seq_len,num_label=num_label,dropout=0.5), ('data',), ('label',)
    
    def sym_gen(seq_len, config = config): ##?返回一个神经网络        
        if 'Big_Jugg_net' in config.configname:
            return Big_Jugg_net(seq_len,num_hidden=256,num_label = num_label,dropout=0.75), ('data',), ('label',)
    ...
    if config.withequdata: ##样本均衡
        data_train = newTextIter(config.path, data_root,  config.batchsize, init_states, num_label, config, buckets=buckets)
    else:
#        data_train = TextIter(config.trainpath, data_root, config.batchsize, init_states, num_label, config, buckets=buckets)
        data_train = TextIter(config.trainpath, data_root, config.batchsize, num_label, init_states, config,)
    
#    data_val = TextIter(config.testpath, test_root, 30, testinit_states, num_label, config, buckets=buckets) ##30是测试集大小
    data_val = TextIter(config.testpath, test_root, 30, num_label, testinit_states, config,) ##30是测试集大小

    model = mx.mod.BucketingModule( ##变长度输入的模型
        sym_gen             = sym_gen, ##A function when called with a bucket key, returns a triple (symbol, data_names, label_names)
        default_bucket_key  = 20, ##The key for the default bucket. ##这里必须 = 4 * max_ratio.(iter文件中的参数)
        context             = config.contexts) ##定义处理器。默认是 mx.cpu()

    prefix = config.configname+'_model/param' ##前缀
    mkdir (cur_path+'/'+config.configname+'_model')
#    prefix = '/data/huzechen/model_yyh/BeijingPk/'+ config.configname+'_model/param' ##前缀
#    mkdir ('/data/huzechen/model_yyh/BeijingPk/'+config.configname+'_model')
  
    if(config.epochload != 0):
        n_epoch_load = config.epochload
        sym, arg_params, aux_params = \
            mx.model.load_checkpoint('/data/huzechen/work_yyh/BeijingPk/Big_Jugg_net_Hebei_pk_model/param',n_epoch_load) ##加载模型。第epochload步的模型
        # sym, arg_params, aux_params = \
        #    mx.model.load_checkpoint('/data/huzechen/work/plate_OCR/Jugg_net_1_2yyh_model/param',n_epoch_load) ##加载模型。第epochload步的模型
    ...
    if(config.function == 'train' or config.function == 'test'):
        model.fit(
                ...
                epoch_end_callback  = mx.callback.do_checkpoint(prefix, 1), ## 每10次保存一次模型
                batch_end_callback  = mx.callback.Speedometer(config.batchsize, 50), ##周期性记录训练速度和精度。每50个batch一次。
