...
class TextIter(mx.io.DataIter):
    def __init__(self, data_path, data_root, batch_size, num_label, 
                 init_states = None, config = None, doubleSingleFlag=False, data_name='data', label_name='label', bucket_key = 5):
        ...
        self.doubleSingleFlag=doubleSingleFlag
        ...
        if self.doubleSingleFlag==False:
            ...
            doubleSingleList=os.listdir(self.data_path)
            for key in doubleSingleList:
                self.data_all[key]=self.read_content(self.data_path+'/'+key+'/')
                self.key_num+=len(self.data_all[key])
                for value in self.data_all[key].values():
                    if len(value)>self.data_length_max:
                        self.data_length_max=len(value)
...
#        self.data_plan = self.get_batch_plan()
    ...
    def get_batch_plan(self): ##考虑是否样本均衡       
        batch_datas = []
        batch_idxes=[]
        if self.doubleSingleFlag==True:
            for key in self.data_all.keys():
                SinDoubdata=self.data_all[key]
                for key2 in SinDoubdata.keys():
                    batch_idxes=random.sample(range(0,len(SinDoubdata[key2])),min(self.batch_size_len,len(SinDoubdata[key2])-1))
                    for idx in batch_idxes:
                        batch_datas.append(SinDoubdata[key2][idx])
        else:
            for key in self.data_all.keys():
                batch_idxes=random.sample(range(0,len(self.data_all[key])),min(self.batch_size_len,len(self.data_all[key])-1))
                for idx in batch_idxes:
                    batch_datas.append(self.data_all[key][idx])
#        print 'batch_len: ',len(batch_datas)
#        print 'first 3 idx: ',batch_idxes[:3]
#        print 'sample: ',batch_datas[1s]
        return batch_datas
#        
#        
#        for key in self.data_all.keys():
...
#        return batch_idxes
    ...
    def get_batch_gray(self, current_batch_idxes): ##一个batch中img路径集合
        ...
        for imgData in current_batch_idxes:
            label_data = imgData[1:-1]
