...
def residual_unit(data, num_filter, stride, dim_match, name, bottle_neck=True, bn_mom=0.9, workspace=256, memonger=False):
    ...
    if bottle_neck:
        ...
        bn1 = mx.sym.BatchNorm(data=data, fix_gamma=False, eps=2e-5, momentum=bn_mom, name=name + '_bn1') ##标准化每批输入。data = （data-data.mean）/sqrt(data.var)
...
def resnet(units, num_stages, filter_list, num_classes, image_shape, bottle_neck=True, bn_mom=0.9, workspace=256, memonger=False):
    ...
    #mx.viz.plot_network(relu1,shape=shape).view()
    return relu1

def get_symbol(num_classes, num_layers, image_shape, conv_workspace=256, net_size = 1, **kwargs):
    """
    Adapted from https://github.com/tornadomeet/ResNet/blob/master/train_resnet.py
    Original author Wei Wu
    """
    image_shape = [int(l) for l in image_shape.split(',')] #image_shape输入的是字符串
    (nchannel, height, width) = image_shape ##和[nchannel, height, width] = image_shape 一样
    if height <= 28:
        num_stages = 3
    ...
    for i in range(len(filter_list)):
        filter_list[i] = int(filter_list[i] * net_size)
